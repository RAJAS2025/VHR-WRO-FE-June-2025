...
int angle = 0;
int SERVO_CENTER = 90;
int CAMERA_CENTER_X = 150;  // depends on resolution
float SCALE = 0.2;          // tuning factor for sensitivity
int MIN_ANGLE = 60;
int MAX_ANGLE = 120;
...
void loop() {
  ...
  if (!huskylens.request()) {
    ...
    if (result.command == COMMAND_RETURN_BLOCK) {
      if (result.ID == 2 && result.yCenter >= 80) { // RED
        Serial.print("Red Detected");
        myservo.write(100);
        delay(500);
        myservo.write(90);
      }
      else if (result.ID == 1 && result.yCenter >= 80) { // GREEN
