...
void loop() {
  ...
  camread();
  if (ra > ga) {
    red = 0.0035 * (20 - rx) * ry;  //0.005
    green = 0;
    curr_y = ry;
  } else if (ga > ra) {
    green = 0.00325 * (280 - gx) * gy;  //0.004
    red = 0;
    curr_y = gy;
  }
  if (pw) {
  ...
  if (bl == true && by > 150 && side == 1 && (millis() - turned > 2000)) {  ////by > 140 && bx > 140
    // motor.stop_motor();
    count += 1;
    if (count >= 13) {
      park();
...
void forward(float g, float block, int rwd, int lwd) {
  if (rwd < 325) {
    rwallval = 0.04 * (rwd - 325);
  } else {
    rwallval = 0;
  }
  if (lwd < 350) {
    lwallval = 0.04 * (lwd - 300);
  } else {
    lwallval = 0;
  }
