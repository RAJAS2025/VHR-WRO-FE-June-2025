...
void loop() {
  ...
  if (pw) {
    ...
    if (side == 1) {
      if (py > gy) {
        ...
        green = 0.0007 * (280 - gx) * gy;
      } else {
        pink = 0.0007 * (270 - px) * py;
        green = 0.003 * (280 - gx) * gy;
      }
    } else {
      if (py > ry) {
  ...
  if (by > gy && by > ry && side == 1 && bx > rx && bx > gx && (millis() - turned > 3500)) {
    green = 0;
    red = 0;
    curr_y = 0;
  }
  ...
  if (bl == true && by > 170 && side == 1 && (millis() - turned > 3500)) {  ////by > 140 && bx > 140
    motor.stop_motor();
    count += 1;
    if (count >= 12) {
      park();
...
void forward(float g, float block, int rwd, int lwd) {
  if (rwd < 250) {
    rwallval = 0.1 * (rwd - 250);
  } else {
    rwallval = 0;
  }
  if (lwd < 250) {
    lwallval = 0.1 * (lwd - 250);
  } else {
    lwallval = 0;
  }
  // kp = 0.8;
