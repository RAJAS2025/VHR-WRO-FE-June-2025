...
void loop() {
  ...
  if (count >= 12) {
    ...
    if (rightdist < 200 && pillar == true) {
      myservo.write(75);
      motor.run_motor(1, 65);

    } else if (leftdist < 200 && pillar == true) {
      myservo.write(115);
      motor.run_motor(1, 65);
    } else {
      // gyro.update();
...
void forward(float g, float block, int rwd, int lwd) {
  if (wallval) {
    ...
    if (rwd < 300) {
      rwallval = 0.02 * (rwd - 300);
    } else {
      rwallval = 0;
    }
    if (lwd < 300) {
      lwallval = 0.02 * (lwd - 300);
    } else {
      lwallval = 0;
    }
  }
  ...
  int pos = 95 - angle;
  pos = constrain(pos, 75, 110);
  myservo.write(pos);
  motor.run_motor(1, 58);
}
