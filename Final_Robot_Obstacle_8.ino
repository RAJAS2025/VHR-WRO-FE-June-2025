...
void loop() {
  ...
  camread();
  if (ra > ga) {
    red = 0.0035 * (0 - rx) * ry;  //0.005
    green = 0;
    curr_y = ry;
  } else if (ga > ra) {
    green = 0.0033 * (280 - gx) * gy;  //0.004
    red = 0;
    curr_y = gy;
  }
  if (pw) {
    ...
    if (side == 1) {
      if (py > gy) {
        pink = 0.0025 * (270 - px) * py;
        green = 0.0003 * (280 - gx) * gy;
      } else {
        pink = 0.0003 * (270 - px) * py;
        green = 0.003 * (280 - gx) * gy;
      }
    } else {
      if (py > ry) {
        pink = 0.0025 * (30 - px) * py;
        red = 0.0003 * (20 - rx) * ry;
      } else {
        pink = 0.0003 * (30 - px) * py;
        red = 0.003 * (20 - rx) * ry;
      }
    }
  } else {
  ...
  if (ol == true && oy > 150 && side == 2 && (millis() - turned > 5000)) {
    // motor.stop_motor();
    count += 1;
    if (count >= 12) {
      park2();
...
void park2() {
  ...
  motor.stop_motor();
  /////////Nackwards straightening
  gyro.update();
  yaw = gyro.getYaw();
  motor.run_motor(0, 45);
  if (abs(yaw) > abs(offsetangle)) {
  ...
  } else if (abs(yaw) < abs(offsetangle)) {
    target = abs(offsetangle) - 24;
    while (yaw > (target * -1)) {
      gyro.update();
      yaw = gyro.getYaw();
