#include <SMotor.h>        // Library for controlling DC motors (custom for WRO kits)
...
SMotor motor(1);           // Create motor object for motor channel 1
Servo myservo;             // Servo object for steering
int servoPin = 27;         // Servo is connected to GPIO pin 27
ZebraGyro gyro(7, 12);     // Gyroscope connected on SDA=7, SCL=12 (I2C)
ZebraTOF front(5);         // Front ToF sensor at address 1
ZebraTOF right(4);         // Right ToF sensor at address 2
ZebraTOF left(3);          // Left ToF sensor at address 3
...
void setup() {
  ...
  motor.run_motor(1, 0);       // Stop motor at the beginning
...
void loop() {
  ...
  if (frontdist < 550 && frontdist > 0 && ((millis() - turned) > 3000)) { // Stop when at a certain distance from front wall
    motor.run_motor(1, 60);              // Run motor to turn
