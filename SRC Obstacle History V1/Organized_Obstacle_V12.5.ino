...
void loop() {
  ...
  if (!huskylens.request()) {
    ...
    if (result.command == COMMAND_RETURN_BLOCK && result.yCenter >= 20) {
      if (result.ID == 1 && result.xCenter >= 10) {  // Red → turn Right
        myservo.write(100);
        ...
        myservo.write(90);
        delay(100);
      } else if (result.ID == 2 && result.xCenter <= 290) {  // Green → turn Left
        myservo.write(80);
...
void rforwardwall(float g, int d) {
  if (d > 600) {
    angle = int(1.7 * (g - (offsetangle + 10)));
  } else if ( d > 450) {
    angle = int(1.2 * (g - (offsetangle + 4)));
  } else if (d < 400) {
    angle = int(1.2 * (g - (offsetangle - 4)));
  } else {
    angle = int(1.7 * (g - (offsetangle)));
  }
  int pos = 90 - angle;
  myservo.write(pos);
}
