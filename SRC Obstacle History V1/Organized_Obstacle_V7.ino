...
void loop() {
  ...
  if (!huskylens.request()) {
    ...
    if (result.command == COMMAND_RETURN_BLOCK && result.yCenter >= 80) {
      int error = result.xCenter - CAMERA_CENTER_X;
      int turnAngle;

      if (result.ID == 1) { // GREEN → turn LEFT
        Serial.println("Red Detected");
        turnAngle = SERVO_CENTER - abs(error * SCALE);
        delay(100);
      } else if (result.ID == 2) { // RED → turn RIGHT
